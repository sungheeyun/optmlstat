...
from ml.modeling.bayesian_least_squares import BayesianLeastSquares
...
class TestBayesianLeastSquares(unittest.TestCase):
    def test_something(self):
        ...
        noise_precision: float = 10.  # beta
        confidence_level: float = 0.90

        coef_1d: np.ndarray = nr.randn(input_dim)
        print(coef_1d)

        prior: Gaussian = Gaussian(np.zeros(input_dim), precision=np.eye(input_dim) * 4.)

        bayesian_least_squares: BayesianLeastSquares = BayesianLeastSquares(prior, noise_precision)
        ...
        for idx in range(num_trainings):
            ...
            y_data_array_1d: np.ndarray = np.dot(x_data_array_2d, coef_1d) + nr.randn(data_size) / np.sqrt(noise_precision)
            y_list.extend(list(y_data_array_1d))

            bayesian_least_squares.train(x_data_array_2d, y_data_array_1d)

        conf_coef: float = ss.norm.ppf(.5 * (1.0 + confidence_level))
        ...
        print(within_interval_bool_array.sum() / within_interval_bool_array.size)

        from matplotlib import pyplot as plt

        fig, ax = plt.subplots()
        ax.plot(x_array, y_array, "-", label="observation")
        ax.fill_between(x_array, lb_array, ub_array, color="b", alpha=0.1, label="prediction interval")
        ax.plot(x_array, y_hat_std_array * conf_coef, '-', label="interval size")
        ax.plot([x_array[0], x_array[-1]], conf_coef / np.sqrt(noise_precision) * np.ones(2), 'r-', alpha=0.1, label="lower limit on prediction interval")
        ax.legend()
        fig.show()
        ...
        for idx in range(547):
            ...
            pred_dist = bayesian_least_squares.get_predictive_dist(new_x_array_1d)

            mu, sigma = pred_dist
            std: float = np.sqrt(sigma)
            true_val: float = np.dot(coef_1d, new_x_array_1d) + nr.randn() / np.sqrt(noise_precision)

            in_list.append(true_val < mu + conf_coef * std and true_val > mu - conf_coef * std)
