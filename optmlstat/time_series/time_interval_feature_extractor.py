...
class TimeIntervalFeatureExtractor(FeatureExtractorBase):
    ...
    def __init__(
        self, start_time: datetime, end_time: datetime, feature_extractor: Callable[[ndarray], ndarray]
    ) -> None:
        self.start_time: datetime = start_time
        self.end_time: datetime = end_time
        self.feature_extractor: Callable[[ndarray], ndarray] = feature_extractor

    def get_features_from_time_series_collection(self, time_series_collection: TimeSeriesCollection) -> ndarray:
        ...
        for time_series in time_series_collection.time_series_list:
            ...
            idx_array: ndarray = logical_and(date_time_index >= self.start_time, date_time_index < self.end_time)
            array_list.append(self.feature_extractor(time_series_data_frame.iloc[idx_array].values.ravel()))
