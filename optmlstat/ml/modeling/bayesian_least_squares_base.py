...
class BayesianLeastSquaresBase(BayesianModelerBase):
    ...
    def solve_linear_sys_using_lower_tri_from_chol_fac(
        ...
        z_array_1d: np.ndarray = cls.forward_substitution(lower_tri, y_array_1d)
        x_array_1d: np.ndarray = cls.backward_substitution(lower_tri.T, z_array_1d)
        return x_array_1d

    @classmethod
    def forward_substitution(cls, lower_tri: np.ndarray, y_array_1d: np.ndarray) -> np.ndarray:
        ...
        for idx in range(vec_size):
            x_array_1d[idx] = (
                y_array_1d[idx] - np.dot(lower_tri[idx, :idx], x_array_1d[:idx])
            ) / lower_tri[idx, idx]

        return x_array_1d

    @classmethod
    def backward_substitution(cls, upper_tri: np.ndarray, y_array_1d: np.ndarray) -> np.ndarray:
        vec_size: int = y_array_1d.size
        ...
        for idx in range(vec_size - 1, -1, -1):
            x_array_1d[idx] = (
                y_array_1d[idx]
                - np.dot(upper_tri[idx, idx + 1 :], x_array_1d[idx + 1 :])  # noqa: E203
            ) / upper_tri[idx, idx]
